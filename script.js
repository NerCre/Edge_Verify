(() => {
  ...
  function updateEntryRRPreview() {
    ...
    // どれか未入力なら空欄（null が 0 扱いになるのを防ぐ）
    if (e === null || tp === null || ls === null) {
      rrEl.value = "";
      return;
    }

    // まずは「価格（絶対値）」として計算
    let rr = computeRR(dir, e, tp, ls);

    // 価格として成立しない場合、TP/SL を「距離」で入力している可能性を救済
    // 例）日経先物: Entry=33000, TP=100, SL=60 のような入力
    if (rr === null && e > 5000 && tp > 0 && ls > 0 && tp < 5000 && ls < 5000) {
      const tpAbs = (dir === "short") ? (e - tp) : (e + tp);
      const lsAbs = (dir === "short") ? (e + ls) : (e - ls);
      rr = computeRR(dir, e, tpAbs, lsAbs);
    }
...
// Entry: image upload (optional)
    ...
    // RR（Entry）: 価格が変わったら自動更新（input / change 両方で拾う）
    ["#entry-price", "#entry-LimitPrice", "#entry-LossPrice"].forEach((sel) => {
      const el = $(sel);
      el?.addEventListener("input", updateEntryRRPreview);
      el?.addEventListener("change", updateEntryRRPreview);
    });
