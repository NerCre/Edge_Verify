...
rrGateEnabled: true,     // RR gate strict（RRが下限未満なら原則ノーポジ）
  ...
  const elExitOnlyOpen = $("#exit-only-open");
    const elExitOnlyUnfinished = $("#exit-only-unfinished");
...
function onJudge(shouldSave) {
  ...
  function updateExitSelect(targetId = null) {
    ...
    const onlyUnfinished = !!(elExitOnlyUnfinished && elExitOnlyUnfinished.checked);

    const listBase = onlyOpen ? sorted.filter(r => !r.hasResult) : sorted;
    const list = onlyUnfinished
      ? listBase.filter(r => normalizeCompletionStatusForJudge(r.completionStatus) === "未完成")
      : listBase;
...
elExitSelect?.addEventListener("change", () => {
    ...
    $("#exit-only-open")?.addEventListener("change", () => updateExitSelect());
        $("#exit-only-unfinished")?.addEventListener("change", () => updateExitSelect());
