(() => {
  ...
  function normalizeNumberString(v) {
    ...
    // Convert full-width digits to half-width (e.g., "１２３．４" -> "123.4")
    s = s.replace(/[０-９]/g, (ch) => String.fromCharCode(ch.charCodeAt(0) - 0xFEE0));
    s = s.replace(/．/g, ".").replace(/，/g, ",");
    // Accept comma as decimal separator (common on some keyboards/locales)
    if (s.includes(",") && !s.includes(".")) s = s.replace(/,/g, ".");
    // Remove thousands separators (e.g., "39,000" -> "39000")
...
function toJpDir(dir) {
  ...
  function computeRR(direction, entryPrice, tpPrice, lsPrice) {
    ...
    if (String(direction) === "flat") return null;
    if (entryPrice === null || entryPrice === undefined) return null;
    if (tpPrice === null || tpPrice === undefined) return null;
    if (lsPrice === null || lsPrice === undefined) return null;
...
// Entry: image upload (optional)
    ...
    // iOSの数値入力は "input" が安定しないケースがあるので change/blur も拾う
    ["input", "change", "blur"].forEach((ev) => {
      $("#entry-price")?.addEventListener(ev, updateEntryRRPreview);
      $("#entry-LimitPrice")?.addEventListener(ev, updateEntryRRPreview);
      $("#entry-LossPrice")?.addEventListener(ev, updateEntryRRPreview);
    });
